/*
Cache: jquery-lazy, thickbox, dt-main, dt-language, dt-main-op-jquery, dt-plugins, waypoints, vc_carousel_js, vc_transition_bootstrap_js, qfe_composer_front_js, wc-cart-fragments, jquery-cookie, bitcommerce, jquery-blockui, wc-add-to-cart, jquery-form, jquery-migrate, jquery-core
*/
/* jquery-core: (http://5c99816b4d469.t73.qifeiye.com/qfy-includes/js/jquery/jquery.js) */
...
/* jquery-migrate: (http://5c99816b4d469.t73.qifeiye.com/qfy-includes/js/jquery/jquery-migrate.min.js) */
...
/* jquery-form: (http://5c99816b4d469.t73.qifeiye.com/qfy-includes/js/jquery/jquery.form.min.js) */
/*!
 * jQuery Form Plugin
...
! function(a) {
	...
	function d() {
		if (a.fn.ajaxSubmit.debug) {
			...
			window.console && window.console.log ? window.console.log(b) : window.opera && window.opera.postError && window.opera
				.postError(b)
		}
	}
	var e = {};
	...
	a.fn.attr2 = function() {
		...
		function c(c) {
			var d, e, f = a.param(c, b.traditional).split("&"),
				g = f.length,
				h = [];
			for (d = 0; g > d; d++) f[d] = f[d].replace(/\+/g, " "), e = f[d].split("="), h.push([decodeURIComponent(e[0]),
		...
		function h(c) {
			...
			function g() {
				...
				try {
					if (m.extraData)
						for (var j in m.extraData) m.extraData.hasOwnProperty(j) && (a.isPlainObject(m.extraData[j]) && m.extraData[j].hasOwnProperty(
							"name") && m.extraData[j].hasOwnProperty("value") ? g.push(a('<input type="hidden" name="' + m.extraData[j].name +
							'">').val(m.extraData[j].value).appendTo(x)[0]) : g.push(a('<input type="hidden" name="' + j + '">').val(m.extraData[
							j]).appendTo(x)[0]));
					m.iframeTarget || (q.appendTo("body"), r.attachEvent ? r.attachEvent("onload", h) : r.addEventListener("load", h,
						!1)), setTimeout(b, 15);
					try {
						x.submit()
					} catch (k) {
			...
			function h(b) {
				if (!s.aborted && !F) {
					if (E = e(r), E || (d("cannot access response document"), b = A), b === z && s) return s.abort("timeout"), y.reject(
						s, "timeout"), void 0;
					...
					if (E && E.location.href != m.iframeSrc || v) {
						...
						try {
							...
							if (d("isXml=" + g), !g && window.opera && (null === E.body || !E.body.innerHTML) && --G) return d(
								"requeing onLoad callback, DOM not available"), setTimeout(h, 250), void 0;
							var i = E.body ? E.body : E.documentElement;
							s.responseText = i ? i.innerHTML : null, s.responseXML = E.XMLDocument ? E.XMLDocument : E, g && (m.dataType =
								"xml"), s.getResponseHeader = function(a) {
								var b = {
									"content-type": m.dataType
								};
								return b[a]
							}, i && (s.status = Number(i.getAttribute("status")) || s.status, s.statusText = i.getAttribute("statusText") ||
								s.statusText);
							var j = (m.dataType || "").toLowerCase(),
								k = /(json|script|text)/.test(j);
							if (k || m.textarea) {
								var l = E.getElementsByTagName("textarea")[0];
								if (l) s.responseText = l.value, s.status = Number(l.getAttribute("status")) || s.status, s.statusText = l.getAttribute(
									"statusText") || s.statusText;
								else if (k) {
									var o = E.getElementsByTagName("pre")[0],
										p = E.getElementsByTagName("body")[0];
									o ? s.responseText = o.textContent ? o.textContent : o.innerText : p && (s.responseText = p.textContent ? p.textContent :
										p.innerText)
								}
						...
						s.aborted && (d("upload aborted"), f = null), s.status && (f = s.status >= 200 && s.status < 300 || 304 === s.status ?
								"success" : "error"), "success" === f ? (m.success && m.success.call(m.context, D, "success", s), y.resolve(s.responseText,
								"success", s), n && a.event.trigger("ajaxSuccess", [s, m])) : f && (void 0 === c && (c = s.statusText), m.error &&
								m.error.call(m.context, s, f, c), y.reject(s, "error", c), n && a.event.trigger("ajaxError", [s, m, c])), n &&
							a.event.trigger("ajaxComplete", [s, m]), n && !--a.active && a.event.trigger("ajaxStop"), m.complete && m.complete
							.call(m.context, s, f), F = !0, m.timeout && clearTimeout(w), setTimeout(function() {
			...
			if (m = a.extend(!0, {}, a.ajaxSettings, b), m.context = m.context || m, o = "jqFormIO" + (new Date).getTime(), m.iframeTarget ?
				(q = a(m.iframeTarget), u = q.attr2("name"), u ? o = u : q.attr2("name", o)) : (q = a('<iframe name="' + o +
					'" src="' + m.iframeSrc + '" />'), q.css({
					position: "absolute",
					top: "-1000px",
					left: "-1000px"
				})), r = q[0], s = {
					...
					abort: function(b) {
						...
						q.attr("src", m.iframeSrc), s.error = c, m.error && m.error.call(m.context, s, c, b), n && a.event.trigger(
							"ajaxError", [s, m, c]), m.complete && m.complete.call(m.context, s, c)
					}
				}, n = m.global, n && 0 === a.active++ && a.event.trigger("ajaxStart"), n && a.event.trigger("ajaxSend", [s, m]),
				m.beforeSend && m.beforeSend.call(m.context, s, m) === !1) return m.global && a.active--, y.reject(), y;
			if (s.aborted) return y.reject(), y;
			t = x.clk, t && (u = t.name, u && !t.disabled && (m.extraData = m.extraData || {}, m.extraData[u] = t.value,
			...
			var D, E, F, G = 50,
				H = a.parseXML || function(a, b) {
					return window.ActiveXObject ? (b = new ActiveXObject("Microsoft.XMLDOM"), b.async = "false", b.loadXML(a)) : b =
						(new DOMParser).parseFromString(a, "text/xml"), b && b.documentElement && "parsererror" != b.documentElement.nodeName ?
						b : null
				},
				...
				J = function(b, c, d) {
					...
					return f && "parsererror" === g.documentElement.nodeName && a.error && a.error("parsererror"), d && d.dataFilter &&
						(g = d.dataFilter(g, c)), "string" == typeof g && ("json" === c || !c && e.indexOf("json") >= 0 ? g = I(g) : (
							"script" === c || !c && e.indexOf("javascript") >= 0) && a.globalEval(g)), g
				};
			return y
		}
		...
		"function" == typeof b ? b = {
				success: b
			} : void 0 === b && (b = {}), i = b.type || this.attr2("method"), j = b.url || this.attr2("action"), k = "string" ==
			typeof j ? a.trim(j) : "", k = k || window.location.href || "", k && (k = (k.match(/^([^#]+)/) || [])[1]), b = a.extend(
				!0, {
					url: k,
		...
		var m = {};
		if (this.trigger("form-pre-serialize", [this, b, m]), m.veto) return d(
			"ajaxSubmit: submit vetoed via form-pre-serialize trigger"), this;
		if (b.beforeSerialize && b.beforeSerialize(this, b) === !1) return d(
		...
		if (this.trigger("form-submit-validate", [q, this, b, m]), m.veto) return d(
		...
		return e.delegation ? (a(document).off("submit.form-plugin", this.selector, b).off("click.form-plugin", this.selector,
				c).on("submit.form-plugin", this.selector, e, b).on("click.form-plugin", this.selector, e, c), this) : this.ajaxFormUnbind()
			.bind("submit.form-plugin", e, b).bind("click.form-plugin", e, c)
		...
		for (var c = [], d = 0, e = this.length; e > d; d++) {
			var f = this[d],
				g = a.fieldValue(f, b);
			null === g || "undefined" == typeof g || g.constructor == Array && !g.length || (g.constructor == Array ? a.merge(c,
				g) : c.push(g))
		}
		return c
		...
		if (void 0 === c && (c = !0), c && (!d || b.disabled || "reset" == e || "button" == e || ("checkbox" == e || "radio" ==
				e) && !b.checked || ("submit" == e || "image" == e) && b.form && b.form.clk != b || "select" == f && -1 == b.selectedIndex))
			return null;
		if ("select" == f) {
		...
		return this.each(function() {
			...
			c.test(d) || "textarea" == e ? this.value = "" : "checkbox" == d || "radio" == d ? this.checked = !1 : "select" ==
				e ? this.selectedIndex = -1 : "file" == d ? /MSIE/.test(navigator.userAgent) ? a(this).replaceWith(a(this).clone(
					!0)) : a(this).val("") : b && (b === !0 && /hidden/.test(d) || "string" == typeof b && a(this).is(b)) && (this.value =
					"")
		})
...
/* wc-add-to-cart: (http://5c99816b4d469.t73.qifeiye.com/qfy-content/plugins/bitcommerce/assets/js/frontend/add-to-cart.min.js) */
jQuery(function(a) {
	return "undefined" == typeof wc_add_to_cart_params ? !1 : void a(document).on("click", ".add_to_cart_button",
		function() {
			var b = a(this);
				...
				return a("body").trigger("adding_to_cart", [b, c]), a.post(wc_add_to_cart_params.ajax_url, c, function(c) {
					if (c) {
						...
						if (typeof(c) == "string") {
							...
							if (tmp.status == "error" && tmp.type == "1") {
								b.removeClass('loading');
								qfy_popinfo_fun('<div style="text-align:center;" ><div style="padding:20px;">' + tmp.msg + '</div></div>',
									wc_add_to_cart_params.i18n_ajax_closetime > 1 ? wc_add_to_cart_params.i18n_ajax_closetime : 1)
								return;
							};
						}
						return d = d.replace("add-to-cart", "added-to-cart"), c.error && c.product_url ? void(window.location = c.product_url) :
							"yes" == wc_add_to_cart_params.cart_redirect_after_add ? void(window.location = wc_add_to_cart_params.cart_url) :
							(b.removeClass("loading"), fragments = c.fragments, cart_hash = c.cart_hash, fragments && a.each(fragments,
									function(b) {
								...
								}), b.addClass("added"), wc_add_to_cart_params.is_cart || 0 != b.parent().find(".added_to_cart").size() /*||b.after(' <a href="'+wc_add_to_cart_params.cart_url+'" class="added_to_cart wc-forward" title="'+wc_add_to_cart_params.i18n_view_cart+'">'+wc_add_to_cart_params.i18n_view_cart+"</a>")*/ ,
								refresh_all_cart_panel(c.htm), qfy_popinfo_fun(
									'<div style="text-align:center;" ><img class="qfy_pop_checkedimg" src="/FeiEditor/images/bitcms/check.png" style="margin-top:20px;margin-bottom:20px;" /><div style="padding-bottom:20px;">' +
									wc_add_to_cart_params.i18n_ajax_title + '</div></div>', wc_add_to_cart_params.i18n_ajax_closetime > 1 ?
									wc_add_to_cart_params.i18n_ajax_closetime : 1), fragments && a.each(fragments, function(b, c) {
									a(b).replaceWith(c)
								}), a(".widget_shopping_cart, .updating").stop(!0).css("opacity", "1").unblock(), a(".shop_table.cart").load(
									d + " .shop_table.cart:eq(0) > *",
									function() {
										a("div.quantity:not(.buttons_added), td.quantity:not(.buttons_added)").addClass("buttons_added").append(
											'<input type="button" value="+" id="add1" class="plus" />').prepend(
											'<input type="button" value="-" id="minus1" class="minus" />'), a(".shop_table.cart").stop(!0).css(
											"opacity", "1").unblock(), a("body").trigger("cart_page_refreshed")
...
/* jquery-blockui: (http://5c99816b4d469.t73.qifeiye.com/qfy-content/plugins/bitcommerce/assets/js/jquery-blockui/jquery.blockUI.min.js) */
/*!
...
/* bitcommerce: (http://5c99816b4d469.t73.qifeiye.com/qfy-content/plugins/bitcommerce/assets/js/frontend/bitcommerce.min.js) */
jQuery(function(a) {
	a(".bitcommerce-ordering").on("change", "select.orderby", function() {
			a(this).closest("form").submit()
		}), a("div.quantity:not(.buttons_added), td.quantity:not(.buttons_added)").addClass("buttons_added").append(
			'<input type="button" value="+" class="plus" />').prepend('<input type="button" value="-" class="minus" />').find(
			".input-text").attr("type", "text"), a("input.qty:not(.product-quantity input.qty)").each(function() {
			var b = parseFloat(a(this).attr("min"));
			b && b > 0 && parseFloat(a(this).val()) < b && a(this).val(b)
		}),
		a(document).on("click", ".plus, .minus", function() {
			var b = a(this).closest(".quantity").find(".qty,.num"),
				c = parseFloat(b.val()),
				d = parseFloat(b.attr("max")),
				e = parseFloat(b.attr("min")),
				f = b.attr("step");
			c && "" != c && "NaN" != c || (c = 0), ("" == d || "NaN" == d) && (d = ""), ("" == e || "NaN" == e) && (e = 0), (
				"any" == f || "" == f || void 0 == f || "NaN" == parseFloat(f)) && (f = 1), a(this).is(".plus") ? b.val(d && (d ==
				c || c > d) ? d : c + parseFloat(f)) : e && (e == c || e > c) ? b.val(e) : c > 0 && b.val(c - parseFloat(f));
		...
		$('.add_to_cart_button-ajax').unbind().bind('click', function() {
			...
			if ($thisbutton.is('.product_type_simple')) {
				...
				$.post("/admin/admin-ajax.php", data, function(response) {
					...
					qfy_popinfo_fun(
						'<div style="text-align:center;" ><img class="qfy_pop_checkedimg" src="/FeiEditor/images/bitcms/check.png" style="margin-top:20px;margin-bottom:20px;" /><div style="padding-bottom:20px;">' +
						i18n_ajax_title + '</div></div>', i18n_ajax_closetime > 1 ? i18n_ajax_closetime : 1)
	...
	$(".product-favorities").click(function() {
		if ($(this).hasClass("featured")) {
			var data = {
				...
				type: "deleteFavoriteGoods",
				"product_id": $(this).attr("data-id")
			};
		} else {
			var data = {
				action: "set_favorites",
				type: "addFavoriteGoods",
				"product_id": $(this).attr("data-id")
			};
		}
			...
			if ($thisbutton.is('.product_type_simple')) {
				...
				$.post("/admin/admin-ajax.php", data, function(response) {
					...
					qfy_popinfo_fun(
						'<div style="text-align:center;" ><img class="qfy_pop_checkedimg" src="/FeiEditor/images/bitcms/check.png" style="margin-top:20px;margin-bottom:20px;" /><div style="padding-bottom:20px;">' +
						wc_add_to_cart_params.i18n_ajax_title + '</div></div>', wc_add_to_cart_params.i18n_ajax_closetime > 1 ?
						wc_add_to_cart_params.i18n_ajax_closetime : 1)
...
function shopping_cart_fun() {
	...
	jQuery(".shopping-cart.e_mouse").on("mouseenter tap", function(e) {
		...
		if (jQuery("#page").width() - ($this.children('.shopping-cart-inner').offset().left - jQuery("#page").offset().left) -
			230 < 0) {
	...
	jQuery(".shopping-cart.e_click .wc-ico-cart").on("click", function(e) {
		...
		if (body_w > 992) {
			...
			if (jQuery("body >.shopping-cart").length > 0) {
				...
				jQuery("body").append(
					'<div class="shopping-cart bitcommerce e_click" id="shopping-cart-bitcommerce" style="position:relative;">' +
					jQuery(this).closest(".shopping-cart").html() + '</div>');
			...
			menuTimeoutHide = setTimeout(function() {
				if ($this.hasClass("dt-hovered")) {
					$this.children('.shopping-cart-inner').stop().addClass("qfyanimated").css({
						"opacity": "1",
						"visibility": "visible",
						"animation-duration": "0.25s",
						"animation-delay": "0s",
						"animation-name": "qfyfadeInLeft"
					});
				}
			}, 350);
			jQuery(".shopping-cart-film").unbind().click(function() {
				if (jQuery('.shopping-cart.e_click').css("opacity") != "0") {
					...
					jQuery('.shopping-cart.e_click .shopping-cart-inner').stop().css({
						"animation-name": "qfyfadeInToRight"
					}).animate({
						"opacity": 0
					}, 250, function() {
						jQuery(this).css("visibility", "hidden");
...
function cart_submit(obj, e) {
	...
	if (isajax > 0) {
		...
		var text1 = form.attr("data-text1") ? form.attr("data-text1") : "æ·»åŠ æˆåŠŸï¼";
		var text2 = form.attr("data-text2") ? form.attr("data-text2") : "åŽ»è´­ç‰©è½¦ç»“ç®—";
		var text3 = form.attr("data-text3") ? form.attr("data-text3") : "æˆåŠŸåŠ å…¥è´­ç‰©è½¦";
		...
		jQuery(obj).closest("form").ajaxSubmit({
			success: function(data) {
				...
				if (obj.status == "success") {
					...
					if (isajax == 2) {
						qfy_popinfo_fun(
							'<div style="text-align:center;" ><img class="qfy_pop_checkedimg" src="/FeiEditor/images/bitcms/check.png" style="margin-top:20px;margin-bottom:20px;" /><div style="padding-bottom:20px;line-height:30px;">' +
							text3 + '</div></div>', 3);
						...
						if (body_w <= 992) {
							...
							if (jQuery("body").hasClass("compose-mode") && jQuery("body >.shopping-cart .setting_cart_inner").length == 0) {
								jQuery("body >.shopping-cart .shopping-cart-inner").append(
									'<div class="controls-element vc-controls bit-controls-element inner" style="line-height: 0px;display:block;"><div class="controls-cc" style="left:50%;top:50%;"><a class="control-btn vc-element-name"><span class="vc-btn-content vo-btn-nomove">è®¾ç½®</span></a><a title="è®¾ç½®" class="control-btn control-btn-edit  " onclick="setting_cart_inner(event,this);"><span class="vc-btn-content"><span class="icon"></span></span></a></div></div>'
								);
							}
							jQuery("body >.shopping-cart").removeClass("e_mouse").addClass("e_click").addClass("dt-hovered");
							jQuery('body >.shopping-cart .shopping-cart-inner').addClass("rightfix qfyanimated").css({
								"opacity": "1",
								"visibility": "visible",
								"animation-duration": "0.25s",
								"animation-delay": "0s",
						...
						var tr = "<tr class='cart_btton_popinfo  qfyanimated' style='position:absolute;opacity:0;" + trtop +
							";animation-duration: 1s; animation-delay: 0s; '><td colspan=2 style='padding:0;'><div style='width:" + (
								tablewidth) +
							"px;min-width:200px;padding:20px 5px 20px 20px;border:2px solid #ddd;background: #fff;margin-top:10px;text-align:left;'> <span style='color:#555555;font-weight:700;font-size:12px;'>" +
							text1 + "</span>&nbsp;&nbsp;<a href='" + url + "'><button type='button' style='background-color:" + bgcolor +
							";border:1px solid " + bgcolor + ";color:" + color + ";padding:4px 9px;font-size:12px;'>" + text2 +
							"</button></a>&nbsp;&nbsp;<span style='cursor:pointer;position:absolute;right: 6px;top: 12px;color: #666;font-size:12px;' class='close_cart'>âœ•</span></div></td></tr>";
						if (body_w > 992) {
							jQuery(".cart_button_table").append(tr);
						} else {
							jQuery(".cart_button_table").prepend(tr);
						}
						jQuery(".cart_btton_popinfo").animate({
							opacity: "1"
						});
					...
					qfy_popinfo_fun(
						'<div style="text-align:center;" ><div class="cart_error" style="padding:20px;line-height:30px;">' + obj.msg +
						'</div></div>', 10);
				} else {
					//æœªçŸ¥å¼‚å¸¸
				}
...
function _cart_update(obj, e) {
	...
	sidebar.block({
		message: null,
		overlayCSS: {
			background: '#fff url(//f.goodq.top/qfy-content/plugins/bitcommerce/assets/images/ajax-loader@2x.gif) no-repeat center',
			backgroundSize: '16px 16px',
			opacity: 0.6
		}
	});
...
function minicart_remove(thisobj, e, remove_item, _qfnonce) {
	...
	jQuery(".cart-product-item[data-itemkey='" + itemkey + "']").animate({
		opacity: "0"
	}, 'fast', function() {
		jQuery(this).remove();
	});
	...
	jQuery.post("/admin/admin-ajax.php", {
		action: "bitcommerce_remove_from_cart",
...
function refresh_all_cart_panel(htm, carthtm) {
	...
	if (htm) {
		...
		if (jQuery(htm).find(".wc-ico-cart").length > 0) {
			if (jQuery(htm).find(".wc-ico-cart").attr("data-num") * 1 > 0) {
				...
				jQuery(".dl-menu-fixedheader .wc-ico-cart .carsize,.qfy-cart-num").html(jQuery(htm).find(".wc-ico-cart").attr(
					"data-num") * 1);
			} else {
	...
	if (jQuery(".loginandregister.site_tooler").length > 0) {
		jQuery(".loginandregister.site_tooler").each(function() {
			...
			jQuery.post("/admin/admin-ajax.php", {
				action: "bit_get_widgetview",
				wid: wid,
				novc: novc
			}, function(msg) {
				$this.replaceWith(msg);
				jQuery(".wf-mobile-visible.mobile-cart .mobile-cart-amount,.dl-menu-fixedheader .carsize").html(jQuery(msg).find(
					".carsize").html());
...
function video_product_play(obj, event) {
	...
	if (bodywidth > 800) {
		top.jQuery.blockUI({
			...
			message: "<iframe allowfullscreen class='videoframe' style='width:100%;height:100%;display:block;border:0;overflow:hidden;' data-height='600' data-controlBarVisibility='hover' data-autoplay='true' src='" +
				url +
...
function get_one_coupon(obj) {
	...
	if (jQuery(obj).hasClass("loading")) {
		...
		jQuery(obj).addClass("loading").append(
			"<div class='loadingdiv' style='width:100%;height:100%;position:absolute;top:0;left:0;;background:url(//f.goodq.top/qfy-content/plugins/bitcommerce/assets/images/ajax-loader@2x.gif) no-repeat center rgba(255,255,255,0.2)'></div>"
		);
	}
	...
	jQuery.post("/admin/admin-ajax.php", data, function(response) {
		...
		if (response == "success") {
			...
			qfy_popinfo_fun('<div style="text-align:center;" ><div style="padding:20px;">' + (p.attr("data-failmsg1") ? p.attr(
				"data-failmsg1") : "ä¼˜æƒ åˆ¸é¢†å–å¤±è´¥ï¼Œè¯·ç™»å½•!") + '</div></div>', 3)
		} else {
			qfy_popinfo_fun('<div style="text-align:center;" ><div style="padding:20px;">' + (p.attr("data-failmsg2") ? p.attr(
				"data-failmsg2") : "ä¼˜æƒ åˆ¸é¢†å–å¤±è´¥") + '</div></div>', 3)
		}
	});
}
...
function bc_change_web_address(obj) {
	var index = $(obj).val();
	$.post("/admin/admin-ajax.php", {
			action: "get_web_address",
			"index": index
		},
		function(rlt) {
			rlt = $.parseJSON(rlt);
	...
	wx.ready(function() {
		wx.openAddress({
			success: function(res) {
				var data = {
					action: "add_web_address",
					"index": -1,
					"set_default": 1,
					"web_address_name": "å¾®ä¿¡åœ°å€"
				};
...
/* jquery-cookie: (http://5c99816b4d469.t73.qifeiye.com/qfy-content/plugins/bitcommerce/assets/js/jquery-cookie/jquery.cookie.min.js) */
/*!
	...
	var t = /\+/g,
		u = e.cookie = function(t, s, a) {
			if (s !== undefined && !e.isFunction(s)) {
				...
				return document.cookie = [n(t), "=", i(s), a.expires ? "; expires=" + a.expires.toUTCString() : "", a.path ?
...
/* wc-cart-fragments: (http://5c99816b4d469.t73.qifeiye.com/qfy-content/plugins/bitcommerce/assets/js/frontend/cart-fragments.min.js) */
jQuery(function(a) {
	if ("undefined" == typeof wc_cart_fragments_params) return !1;
	if ($supports_html5_storage = "sessionStorage" in window && null !== window.sessionStorage, $fragment_refresh = {
			url: wc_cart_fragments_params.ajax_url,
			type: "POST",
			data: {
				action: "bitcommerce_get_refreshed_fragments"
			},
			success: function(b) {
				b && b.fragments && (a.each(b.fragments, function(b, c) {
					a(b).replaceWith(c)
				}), $supports_html5_storage && (sessionStorage.setItem(wc_cart_fragments_params.fragment_name, JSON.stringify(
					b.fragments)), sessionStorage.setItem("wc_cart_hash", b.cart_hash)), a("body").trigger(
					"wc_fragments_refreshed"))
			}
		}, $supports_html5_storage) {
		a("body").bind("added_to_cart", function(a, b, c) {
			sessionStorage.setItem(wc_cart_fragments_params.fragment_name, JSON.stringify(b)), sessionStorage.setItem(
				"wc_cart_hash", c)
		});
		try {
			var b = a.parseJSON(sessionStorage.getItem(wc_cart_fragments_params.fragment_name)),
				c = sessionStorage.getItem("wc_cart_hash"),
				d = a.cookie("bitcommerce_cart_hash");
			if ((null == c || void 0 == c || "" == c) && (c = ""), (null == d || void 0 == d || "" == d) && (d = ""), !b || !b[
					"div.widget_shopping_cart_content"] || c != d) throw "No fragment";
	...
	a.cookie("bitcommerce_items_in_cart") > 0 ? a(".hide_cart_widget_if_empty").closest(".widget_shopping_cart").show() :
		a(".hide_cart_widget_if_empty").closest(".widget_shopping_cart").hide(), a("body").bind("adding_to_cart", function() {
			a(".hide_cart_widget_if_empty").closest(".widget_shopping_cart").show()
		})
});;

/* qfe_composer_front_js: (http://5c99816b4d469.t73.qifeiye.com/qfy-content/plugins/qfy_editor/assets/js/qfy_editor_front.js) */
			...
			jQuery('.content-wrapper .qfy-listcatecontrols li').each(function() {
				...
				$li.mouseenter(function() {
					...
					if (jQuery(this).find(".toEditor").length == 0) {
						jQuery(this).append(
							"<span class='toEditor' style='border:0;padding:0;'><span  style='display:inline;border:0;padding:0;background:transparent;' onclick='toDeleteCate(this)'><img src='//f.goodq.top/FeiEditor/bitSite/images/close_hover.png' /></span>"
						);
					}
				}).mouseleave(function() {
...
/* vc_transition_bootstrap_js: (http://5c99816b4d469.t73.qifeiye.com/qfy-content/plugins/qfy_editor/assets/lib/vc_carousel/js/transition.js) */
+
function($) {
	"use strict";
...
/* vc_carousel_js: (http://5c99816b4d469.t73.qifeiye.com/qfy-content/plugins/qfy_editor/assets/lib/vc_carousel/js/vc_carousel.js) */
/* ========================================================================
...
/* waypoints: (http://5c99816b4d469.t73.qifeiye.com/qfy-content/plugins/qfy_editor/assets/lib/jquery-waypoints/waypoints.min.js) */
...
/* dt-plugins: (http://5c99816b4d469.t73.qifeiye.com/qfy-content/themes/qfy-01/js/plugins.js) */
/*jquery-parallax*/
/*scroll ç”¨åˆ°*/
...
/* dt-main-op-jquery: (http://5c99816b4d469.t73.qifeiye.com/FeiEditor/bitSite/js/opentip-jquery.min.js) */
// Opentip v2.4.6
...
/* dt-language: (http://5c99816b4d469.t73.qifeiye.com/qfy-content/themes/qfy-01/js/language.js) */
jQuery(function() {
...
/* dt-main: (http://5c99816b4d469.t73.qifeiye.com/qfy-content/themes/qfy-01/js/main.js) */
//æ‚¬æµ®æ•ˆæžœ
(function($) {
	...
	if (smartMenu && $("#header").length > 0) {
		...
		try {
			if (self != top && !top.jQuery("body").hasClass("caterole")) {
				...
				mouseHtml =
					'onmouseover="document.getElementById(\'phantomFloatMenu\').style.display = \'block\';floatmenucontrols_mouseenter();" onmouseout="document.getElementById(\'phantomFloatMenu\').style.display = \'none\';floatmenucontrols_mouseout();"  ';
				controlDiv =
					'<div id="phantomFloatMenu"  style="line-height: 0px; opacity: 1; visibility: visible; transition: auto 0s ease 0s; display: none;" class="controls-element vc-controls bit-controls-element">' +
					pcontroldiv +
					'<div class="controls-cc"><a class="control-btn vc-element-name"><span class="vc-btn-content vo-btn-nomove">æµ®åŠ¨èœå•å’ŒLOGO</span></a><a class="control-btn control-btn-edit " onclick="top.qfFloatMenuSetting()" title="è®¾ç½® æµ®åŠ¨èœå•æ ·å¼"><span class="vc-btn-content"><span class="icon"></span></span></a></div></div>';
			}
		} catch (e) {
		...
		var scrTop = 0,
			...
			$phantom = $('<div id="phantom" ' + mouseHtml + ' >' + controlDiv +
				'<div class="ph-wrap"><div class="ph-wrap-content"><div class="ph-wrap-inner"><div class="logo-box" ></div><div class="menu-box" ></div><div class="menu-info-box"  ></div></div></div></div></div>'
			).appendTo("body"),
		...
		if (dtGlobals.logoURL && dtGlobals.logoEnabled) {
			...
			if (dtGlobals.logoURL) {
				...
				var img_str = "";
				if (logoURL) {
					img_str = '<a href="' + logoURL + '"><img src="' + dtGlobals.logoURL + '" height="' + dtGlobals.logoH +
						'" width="' + dtGlobals.logoW + '"></a>';
				} else {
					img_str = '<img src="' + dtGlobals.logoURL + '" height="' + dtGlobals.logoH + '" width="' + dtGlobals.logoW + '">';
				}
				$phantom.find(".logo-box").html('<div style="height:48px;vertical-align: ' + valign + ';display: table-cell;" >' +
					img_str + '</div>');
			}
			//}else{
			...
			if (tempScrTop > threshold && isMoved === false) {
				if (!dtGlobals.isHovering) {
					...
					if (jQuery("#header.newrightmenu,#header.newleftmenu").length > 0) {
						if (jQuery("#dl-menu:visible").length == 0 && (jQuery("body").height() - threshold - tempScrTop > jQuery(window)
								.height())) {
						...
						$header.removeClass($header.data('bit-menu')).addClass($header.data('bit-float-menu')).removeClass(
							"bit-menu-default").addClass("bit-menu-float");
					}
				...
				if (jQuery("#header.newrightmenu,#header.newleftmenu").length > 0) {
					...
					$header.removeClass($header.data('bit-float-menu')).addClass($header.data('bit-menu')).addClass(
						"bit-menu-default").removeClass("bit-menu-float");
				}
	...
	if ($("#mobile-menu").hasClass("floatmenu") || $("#mobile-menu").hasClass("fullfloatmenu") || $("#mobile-menu").hasClass(
		...
		if ($("#mobile-menu").hasClass("fullfloatmenu")) {
			...
			if ($("body >.dl-menu-fixedheader").length == 0 && $(".leftbtnmenu #mobile-menu.displaynone").length == 0) {
				...
				if ($(".yy_website_notice").length > 0) {
					$(".yy_website_notice").after("<div class='dl-menu-fixedheader wf-mobile-visible'>" + jQuery(
						".menu_displaynone_header").html() + "</div>");
				} else {
...
function searchqfylist(tag, paged, flag) {
	if (!is_edit_model) {
		...
		jQuery('.qfytemplateslist .viewmoretemplate').append(
			"<div class='loadingimage' style='text-align:center;margin:50px auto;'><img style='vertical-align: middle;width:30px;' src='//f.goodq.top/FeiEditor/bitSite/images/preloader.gif' /><span style='color:#0088c2;margin-left:10px;'>æ­£åœ¨è‡ªåŠ¨åŠ è½½æ›´å¤šæ¨¡æ¿...</span></div>"
		);
...
function checkAdvertising() {
	...
	var style =
		'display:block !important;opacity: 1 !important;position:fixed !important;bottom:40px !important;left:0 !important;width:116px !important;height:25px !important;line-height:24px !important;background:#23282D !important;z-index:2147483647 !important;text-align:center;color:#fff !important;font-size:12px;border-top-right-radius:3px;border-bottom-right-radius:3px;';
	if (!jAdObj.hasClass("band")) {
		var content = 'BY èµ·é£žé¡µè‡ªåŠ©å»ºç«™';
	} else {
		var content = 'èµ·é£žé¡µæä¾›å…è´¹æµé‡';
	}
	if (jAdObj.length == 0) {
		var tmphtml = '<a class="wf-mobile-hidden qfy_advertising" target=_blank style="' + style +
			'"  rel="external nofollow" href="' + href + '" >' + content + '</a>';
...
function qfy_template_preiview_fun(url, select_url) {
	...
	header_str +=
		'<div class="demo-mobile" ><div class="demo-mobile-header"><div class="circle"></div><div class="circle" style="width: 60px;"></div></div><div class="demo-mobile-bottom"></div></div>';

	jQuery("body").append(
		"<div id='qfy_template_preivew' class='pc-selected'><div id='qfy_template_preivew_header'><span style='float:left;' > <span class='back' style='position: relative;top:-6px;font-size:12px;'><svg style='position: relative;top:2px;margin-right:3px;stroke: #222;stroke-width: 7px;width: 14px;overflow: visible;-webkit-transform: scaleX(-1);transform: scaleX(-1);' width=\"8px\" height=\"13px\" viewBox=\"0 0 8 13\" version=\"1.1\"><g style='fill: #222;' id=\"Visual-Design\" stroke=\"none\" stroke-width=\"1\" fill=\"none\" fill-rule=\"evenodd\" opacity=\"1\"><g id=\"9b\" transform=\"translate(-344.000000, -357.000000)\" fill=\"#222222\"><g id=\"templates\" transform=\"translate(-202.000000, 165.000000)\"><path d=\"M544,195 L544,203 L543,203 L543,195 L543,194 L552,194 L552,195 L544,195 Z\" id=\"Combined-Shape\" transform=\"translate(547.500000, 198.500000) rotate(-225.000000) translate(-547.500000, -198.500000) \"></path></g></g></g></svg>è¿”å›ž </span> <svg class='pc'  width=\"28\" height=\"22\" viewBox=\"0 0 28 22\" xmlns=\"http://www.w3.org/2000/svg\"><g fill=\"#000\" fill-rule=\"evenodd\"><path d=\"M11 18h1v4h-1z\"></path><path d=\"M9 21h10v1H9z\"></path><path d=\"M16 18h1v4h-1z\"></path><path d=\"M1 3v13c0 1.11.891 2 1.996 2h22.008A2.004 2.004 0 0 0 27 16V3c0-1.11-.891-2-1.996-2H2.996A2.004 2.004 0 0 0 1 3zM0 3c0-1.657 1.35-3 2.996-3h22.008A2.994 2.994 0 0 1 28 3v13c0 1.657-1.35 3-2.996 3H2.996A2.994 2.994 0 0 1 0 16V3z\"></path></g></svg><svg class='mobile' width=\"12\" height=\"22\" viewBox=\"0 0 12 22\" xmlns=\"http://www.w3.org/2000/svg\"><g fill=\"#000\" fill-rule=\"evenodd\"><path d=\"M1 3.001V19C1 20.105 1.894 21 2.997 21h6.006A2 2 0 0 0 11 18.999V3A1.999 1.999 0 0 0 9.003 1H2.997A2 2 0 0 0 1 3.001zm-1 0A3 3 0 0 1 2.997 0h6.006A2.999 2.999 0 0 1 12 3.001V19A3 3 0 0 1 9.003 22H2.997A2.999 2.999 0 0 1 0 18.999V3z\"></path><path d=\"M5 18h2v2H5z\"></path></g></svg> </span>  <span  style='float:right;'><a class='preivew' target='_blank' href='" +
		url + "'>è®¿é—®æ¼”ç¤ºç½‘ç«™</a><a href='" + select_url +
		"'><button class='use'>é€‰ç”¨</button></a><svg class='back'  style=\"width: 40px;height: 40px;fill:none;stroke:#000000;stroke-miterlimit:3;stroke-width:1px;position: relative;top: 15px;margin-right: 0;margin-left: 20px;\"  viewBox=\"0 0 150 150\" ><line  x1=\"8.15\" y1=\"141.18\" x2=\"141\" y2=\"8.33\"/><line  x1=\"8.15\" y1=\"8.33\" x2=\"141\" y2=\"141.18\"/></svg></span></div><div id='qfy_template_preivew_content' ><div class='iframe-inner'>" +
...
jQuery(document).ready(function($) {
	...
	if (current_page_id != '' && current_page_id != 'undefined' && current_page_key != '' && current_page_key !=
		'undefined') {
		// jQuery.ajax({
...
function play_qfy_video(obj) {
	...
	if (ispop == "1") {
		...
		jQuery(obj).removeClass("toplay").addClass("playing").attr("src",
			"https://f.goodq.top/qfy-content/plugins/qfy_editor/projekktor/themes/maccaco/buffering.gif");
...
function submit_aff_info(obj) {
	...
	var postdata = {
		"action": "update_useraff_info",
		"aff_ff_id": aff_ff_id,
		"aff_ff_name": aff_ff_name,
		"aff_ff_bank": aff_ff_bank,
		"aff_ff_type": aff_ff_type
	};
...
/* thickbox: (http://5c99816b4d469.t73.qifeiye.com/qfy-includes/js/thickbox/thickbox.js) */
/*
...
/* jquery-lazy: (http://5c99816b4d469.t73.qifeiye.com/qfy-includes/js/jquery.lazy.min.js) */
